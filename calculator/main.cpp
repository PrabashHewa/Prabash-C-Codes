...
const vector<Command> COMMANDS = {
    ...
    {"/", 2, false, division},
    {"^", 2, false, exponentiation},  // New exponentiation command
    {"POWER", 2, false, exponentiation},  // New alias for exponentiation
    {"EXP", 2, false, exponentiation},  // New alias for exponentiation
...
int main() {
    ...
    while ( true ) {
        ...
        for (char& ch : command_to_be_executed) {
            ch = toupper(ch);
        }

        // Find the command in the COMMANDS vector
        ...
        // Check parameter count: `pieces.size() - 1` gives us the number of parameters
        if (pieces.size() - 1 != matched_command->parameter_number) {
            cout << "Error: wrong number of parameters." << endl;
            continue;
        }

        // Convert parameters to double and validate them
        ...
        // Execute the command's action and print the result
        if (matched_command->action != nullptr) {
            double result = matched_command->action(left, right);
            cout << result << endl;
        }
    }
}
